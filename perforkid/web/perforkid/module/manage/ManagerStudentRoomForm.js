...
uploadButton.addEventListener("submit", function (e) {
  ...
  if (fileInput.files.length > 0) {
    ...
    if (
      ...
      alert(
        `กรุณาอัปโหลด ไฟล์ชื่อ form_student_${front}-${back}.xlsx เท่านั้น`
      );
    }
  }
  ...
  if (fileTable.files.length > 0) {
    ...
    if (
      jpgfile &&
      jpgfile.name.endsWith(".jpg") &&
      jpgfile.name.startsWith(`student_${front}-${back}`)
    ) {
...
async function fetchLatestStudentUpdate(room, currentUser) {
  ...
  if (!querySnapshots.empty) {
    ...
    const studentQuerySnapshot = await getDocs(
      query(studentRef, where("room", "==", room))
    );
...
const latestStudentUpdateElement = document.getElementById(
  "latestStudentUpdate"
);
if (latestStudentUpdateElement) {
  latestStudentUpdateElement.innerText =
    "( Latest student Update: " + latestStudentUpdate + " )";
} else {
  latestStudentUpdateElement.innerText =
    "( Latest student Update: No upload history found. )";
}
...
function upload(file) {
  ...
  const storageRef = ref(
    storage,
    `school/${school}/form_student/year${front}/${newFileNameFormatted}`
  );
                    ...
                    .then((studentSnapshot) => {
                      studentSnapshot.forEach((studentDoc) => {
                        ...
                        const studentListRef =
                          studentDoc.ref.collection("student-list");
                        ...
                        studentListRef
                          ...
                          .then((studentListSnapshot) => {
                            ...
                            studentDoc.ref.update({
                              "latest-student-update": moment().format(
                    ...
                    .then((studentSnapshot) => {
                      if (studentSnapshot.empty) {
                        ...
                        studentRef
                          ...
                          .then((newStudentDoc) => {
                            const studentListRef =
                              newStudentDoc.collection("student-list");
                            // อัปโหลดข้อมูลจาก JSON ไปยัง 'student-list'
                            jsonData.forEach((item) => {
                              //item.name = "student";
                              studentListRef.add(item);
...
async function uploadTable(file, roomNumber) {
  ...
  const storageRef = ref(
    storage,
    `school/${school}/form_student_table/year${front}/${newFileNameFormatted}`
  );
  ...
  if (file) {
    ...
    uploadBytes(storageRef, file)
      .then((snapshot) => {
        ...
        alert(
          "อัปโหลดไฟล์สำเร็จแล้ว! อีกสักครู่ระบบจะทำการรีเฟรชหน้าจออัตโนมัติ"
        );
  ...
  const schoolQuerySnapshot = await getDocs(
    query(schoolRef, where("school-name", "==", schoolName))
  );
  if (!schoolQuerySnapshot.empty) {
    ...
    const StudentRoomQuerySnapShot = await getDocs(
      query(RoomRef, where("room", "==", roomNumber))
    );
